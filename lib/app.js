...
function App(config) {
  ...
  this.meteorApp;
  var laikaInjectPort;
  this.getSettings = function (filename) { //This function is stolen straight from meteor, we do the same settings parsing
    var str;
    try {
  ...
  this.start = function start(dbname, port) {
    ...
    process.env.METEOR_SETTINGS = this.getSettings(config.settings);
    ...
    this.meteorApp.stdout.on('data', function(data) {
      ...
      if(data.match(/^error/i)) {
        var message = '[ Server Crashed ] ' + data;
